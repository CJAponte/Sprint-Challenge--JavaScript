...
lowerCase.push(zooAnimals.map((el)=>{
...
largerPopulation.push(zooAnimals.filter((el)=>{
  if(el.population < 5){
    return el
  }
}))
...
(()=>{
  let tempPopulationStorage = []
  for(let i = 0; i < zooAnimals.length; i++){
    tempPopulationStorage.push(zooAnimals[i].population)
  }
  populationTotal = tempPopulationStorage.reduce((accum, current)=>{
